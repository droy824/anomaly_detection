...
class deepsort_rbc():
	...
	def run_deep_sort(self, frame, out_scores, out_boxes):
		...
		print("170", processed_crops.is_cuda)
		device = torch.device("cuda" if torch.cuda.is_available() else "cpu")  # Select device for inference
		processed_crops = processed_crops.to(device)
		print("172", processed_crops.is_cuda)
		processed_crops = self.gaussian_mask * processed_crops
		print("174", processed_crops.is_cuda)
		print("175", self.gaussian_mask.is_cuda)
